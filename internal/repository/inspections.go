...
func (r Inspections) CreateTx(ctx context.Context, tx *sql.Tx, i domain.Inspection) error {
	_, e := tx.ExecContext(ctx, `INSERT INTO inspections(id,task_id,inspector_id,result,score,notes,created_at) VALUES(?,?,?,?,?,?,?)`, i.ID, i.TaskID, i.InspectorID, i.Result, i.Score, i.Notes, i.CreatedAt.UTC().Format(time.RFC3339))
	return e
}
