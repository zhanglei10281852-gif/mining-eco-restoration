...
func (r Tasks) ReserveInspection(ctx context.Context, id, from, to string, version int64, at time.Time) (bool, error) {
	res, err := r.DB.ExecContext(ctx, `UPDATE remediation_tasks SET status=?,version=version+1,updated_at=? WHERE id=? AND status=? AND version=?`, to, at.UTC().Format(time.RFC3339), id, from, version)
	if err != nil {
		return false, err
	}
...
func (r Tasks) ReserveInspectionTx(ctx context.Context, tx *sql.Tx, id, from, to string, version int64, at time.Time) (bool, error) {
