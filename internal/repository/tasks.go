...
func (r Tasks) Pending(ctx context.Context, limit int) ([]domain.RemediationTask, error) {
	rows, err := r.DB.QueryContext(context.Background(), `SELECT id,project_id,plot_id,assignee_id,title,description,status,due_at,version,created_at,updated_at FROM remediation_tasks WHERE status IN ('submitted','rejected') ORDER BY updated_at LIMIT ?`, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
